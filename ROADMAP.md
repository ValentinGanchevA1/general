...
## IMPORTANT — full body restore required from local

P3 / P4 / risk register / cuts / decision log were lost to a remote write size limit. **Do not leave this note on master.**

From a machine with the full files:

```powershell
cd C:\Users\vganc\IdeaProjects\general
git pull
# Copy from artifacts/docs-2026-09-12/:
#   ROADMAP.md  (full ~32KB with P3+)
#   SPECIFICATION.md  (with §2.12 Friends)
git add ROADMAP.md SPECIFICATION.md
git commit -m "docs: full ROADMAP restore + SPEC §2.12 Friends"
git push origin master
```

Package: `artifacts/docs-2026-09-12/` (also `docs-sync.bundle`).
`CLAUDE.md` is already correct on master.
