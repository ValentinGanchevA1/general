...
export class ListingsService {
  ...
  async detail(userId: string, listingId: string): Promise<ListingDetail> {
    ...
    const [mine] = (await this.db.query(
      `SELECT o.id, o.listing_id, o.buyer_id, o.offer_cents, o.message, o.status, o.created_at, o.last_actor,
  ...
  /** Seller counter: keep status=pending, update amount, mark last_actor=seller. */
  async counterOffer(
    sellerId: string,
    offerId: string,
    dto: CounterOfferDto,
    ...
    const [row] = (await this.db.query(
      `SELECT o.id, o.status, o.listing_id, l.seller_id, l.status AS listing_status
         FROM trade_offers o
         JOIN listings l ON l.id = o.listing_id AND l.deleted_at IS NULL
        WHERE o.id = $1`,
      [offerId],
    )) as Array<{
      id: string;
