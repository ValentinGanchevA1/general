...
export class StoriesService {
  ...
  private async assertCanPost(userId: string): Promise<void> {
    const [u] = (await this.db.query(
      `SELECT verification_level FROM users
        WHERE id = $1 AND deleted_at IS NULL`,
      [userId],
    )) as Array<{ verification_level: VerificationLevel }>;
    if (!u) {
      throw new ForbiddenException({ code: 'auth.forbidden', message: 'User not found.' });
    }
    const allowed: VerificationLevel[] = ['phone', 'selfie', 'id'];
    if (!allowed.includes(u.verification_level)) {
      throw new ForbiddenException({
        code: 'story.phone_required',
        message: 'Phone verification required to post stories.',
      });
    }
  }
