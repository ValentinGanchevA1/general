...
export class AuthService {
  ...
  async appleOAuth(identityToken: string, fullName?: string, emailHint?: string): Promise<LoginResponse> {
    // Lazy-import to avoid bundling the JWK client unless Apple OAuth is used.
    const { default: appleSignin } = await import('apple-signin-auth') as { default: {
      verifyIdToken: (token: string, opts: Record<string, unknown>) => Promise<{ sub: string; email?: string }>;
    }};
    ...
    try {
      ...
      // Apple only includes email in the token on first sign-in.
      email = payload.email ?? emailHint;
    } catch {
      throw new UnauthorizedException({ code: 'auth.oauth_failed', message: 'Apple token verification failed' });
    }

    if (!email && !appleSub) {
      throw new UnauthorizedException({ code: 'auth.oauth_failed', message: 'Apple token missing required claims' });
    }

    const conditions: string[] = ['apple_sub = $1'];
    const params: unknown[] = [appleSub];
    if (email) {
    ...
    if (found) {
      user = found;
      await this.db.query(
        `UPDATE users SET apple_sub = $1 WHERE id = $2 AND apple_sub IS NULL`,
        [appleSub, user.id],
      );
    } else {
      if (!email) {
        // No existing account and Apple withheld the email (not first sign-in).
        // Can't create an account without an email.
        throw new UnauthorizedException({ code: 'auth.apple_no_email', message: 'Email required for first Apple sign-in' });
      }
