...
export function UserProfileScreen({ route, navigation }: Props): React.JSX.Element {
	...
	const sectionY = useRef<Partial<Record<'trust' | 'stats' | 'storyline' | 'photos' | 'bio' | 'mutual', number>>>({});
	...
	// Reset focus-scroll gate when opening a different profile.
	useEffect(() => {
		didFocusScroll.current = false;
	}, [userId, focus]);

	// Mutual friends focus opens the dedicated list (same as sheet mutual row).
	useEffect(() => {
