...
function SellerControls({
	...
	const onBump = useCallback(async () => {
		if (bumping || status !== 'active') return;
		setBumping(true);
		try {
			await bumpListing(listingId);
			onChanged();
			appAlert('Listing bumped', 'Expiry extended for nearby buyers.');
		} catch (e) {
			const err = e as ApiError;
			appAlert(
				err.code === 'listing.bump_cooldown' ? 'Bump cooling down' : 'Could not bump',
				err.message || 'Try again later.',
			);
		} finally {
			setBumping(false);
		}
	}, [bumping, status, listingId, onChanged]);
			...
			<View style={S.sellerActions}>
				{status === 'active' ? (
					<>
						...
						>
							<Text style={S.secondaryBtnText}>{bumping ? '…' : 'Bump'}</Text>
						</TouchableOpacity>
