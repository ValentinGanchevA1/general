...
export function FriendsListScreen(): React.JSX.Element {
  ...
  const loading = tab === 'friends' ? listState.listLoading : listState.requestsLoading;
  const error = tab === 'friends' ? listState.listError : listState.requestsError;
  const nextCursor = tab === 'friends' ? listState.listNextCursor : listState.requestsNextCursor;
  const loadingMore = tab === 'friends' ? listState.listLoadingMore : listState.requestsLoadingMore;

  const onRefresh = useCallback(() => {
    if (tab === 'friends') void dispatch(fetchFriends({}));
    else {
      void dispatch(fetchRequests({}));
      void dispatch(fetchPendingCount());
    }
  ...
  const onEndReached = useCallback(() => {
    ...
    if (tab === 'friends') void dispatch(fetchFriends({ cursor: nextCursor }));
    else void dispatch(fetchRequests({ cursor: nextCursor }));
  }, [dispatch, tab, nextCursor, loadingMore]);
