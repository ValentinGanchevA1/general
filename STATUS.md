...
> **Current phase:** P5 feature build-out — **Gifts (XP-funded, v1)** ✅ shipped end-to-end (backend + mobile + realtime/push), Challenges mobile screen shipped. P4 profile & monetization surface (G1–G5) code-complete; **all migrations applied to prod through `0019`**; **G2 (Twilio) + G3 (Stripe, test mode) creds landed 2026-06-05** (pending live verify), G4 social deferred. Apple Sign-In (A3) removed from scope 2026-06-05 (0019 applied). **CI Backend/Mobile jobs fixed 2026-06-05 — repo CI green.** P2 7-day synthetic gate clears 2026-06-06.
> **Owner:** [your name]
>
...
**Deploy checklist (P4):**
- ✅ **Migrations** — 0001–0018 all applied to prod Supabase (verified live 2026-06-05 against `schema_migrations`; the earlier "run 0012–0016" line was stale — they were applied 2026-05-31).
...
> **Migrations:** all 0001–0015 are idempotent (guarded DDL). The former `0012` prefix collision is resolved — the achievements migration moved to `0015_achievements.sql` (no deps, latest feature), while `0012_profile_expansion` stays ahead of `0013`/`0014` which depend on its columns. `schema_migrations` rows renamed in lockstep. **All migrations through `0019` are applied to prod Supabase** (verified live 2026-06-05). Next free number is `0020` (`0016` = drop VIP tier, `0017` = message requests, `0018` = gifts, `0019` = drop Apple OAuth — applied to prod 2026-06-05).
