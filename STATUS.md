...
**Deploy checklist (P4):** run migrations 0012–0015; set `TWILIO_ACCOUNT_SID/AUTH_TOKEN/VERIFY_SERVICE_SID`, `STRIPE_SECRET_KEY/WEBHOOK_SECRET/PRICE_{BASIC,PREMIUM,VIP}` (+ optional URLs), `{PROVIDER}_CLIENT_ID/SECRET` + `API_PUBLIC_URL` + `SOCIAL_LINK_RETURN_URL`; register Stripe webhook → `/api/v1/subscriptions/webhook` and provider redirects → `/api/v1/social/callback`.
